...
5. **自动类型转换**
    ...
    如果某些数据不能够自动转换时，则保留在定义该对象类时设置的初始值。例如：
    ...
    # 由于不能将 "19.a" 转换为 int，保留了在定义 age 时设置的默认值。
    # Child(name='Hjz', age=0)
    ```

由于没有找到很复杂的数据来测试，我自己也比较懒，所以上面的案例其实是我在写代码的时候应用的测试数据，可能不够复杂。

如果有小伙伴在使用的时候，发现数据无法解析可以给我留言，让我们一起改善这个模块。感谢支持！
